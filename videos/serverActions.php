...
$subject = "LucasPiano.com | Coment�rio postado no Site";
...
<table width='500' style='border:1px solid #666666;' cellspacing='0' cellpadding='0'>
  <tr style='font-weight:bold; font-family: Tahoma, Verdana, Arial, sans-serif; font-size:12px; color:#FFF; background-color:#00569D;'>
    <td style='padding:10px;'>Coment�rio postado no Site | ".date("d-m-Y")."</td>
  </tr>
  <tr>
...
//Recupera o Modulo e a a�ao do Form (inclusao ou alteracao) e define a a��o do SQL
...
if (empty($nome) or empty($comentario) or empty($email)) {
	echo "<script>alert('Nome, email e coment�rio devem ser preenchidos!');</script>";
	echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=video.php?codigo=".$codigoVideo."'>";
	exit();
}

if (empty($codigoVideo)) {
	echo "<script>alert('Ocorreu um erro ao postar seu coment�rio. Por Favor, tente mais tarde.');</script>";
...
// Define a Query para inclus�o no bd
